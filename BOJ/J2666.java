...
public class J2666 {
    ...
    public static void main(String[] args)throws IOException{
        ...
        closet = new int[n];
        open = Arrays.stream(br.readLine().split(" ")).mapToInt(v -> Integer.parseInt(v)).toArray();
        ...
        System.out.println(solve(open[0],open[1],query[0],1));
    }

    public static int solve(int l,int r, int x,int c){
        if(c == m) return 0;
        ...
        closet[l] = 0;
        closet[x] = 1;
        lrt += solve(x,r,query[c],c+1);

        int rrt = Math.abs(x-r);
        closet[l] = 1;
        closet[r] = 0;
        rrt += solve(l,x,query[c], c+1);
